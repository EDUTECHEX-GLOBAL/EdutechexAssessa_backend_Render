// backend/controllers/satStudyPlanController.js
...
const SatAssessmentUpload = require("../models/webapp-models/satAssessmentModel");
...
const getAITip = (feedbacks) => {
  ...
  try {
    const feedbackText =
      typeof feedbacks[0].feedbackText === "string"
...
const getPriorityIcon = ({ completion, marksLost, accuracy }) => {
  if (completion >= 80 || accuracy >= 80) return "✅";
  if (completion < 50 || accuracy < 60) return "⚠️";
  if (completion === 0 && marksLost > 20) return "❗";
  return "📘";
};

// ========== Controllers ==========

// Generate SAT study plan
const getStudyPlan = asyncHandler(async (req, res) => {
  try {
    const { studentId } = req.params;
    await validateStudent(studentId);

    const completedTasks = await SatStudyPlan.find({
      studentId,
      isCompleted: true,
    }).lean();
    ...
    const wrongAnswers = await SatSubmission.aggregate([
      { $match: { studentId: new mongoose.Types.ObjectId(studentId) } },
      { $unwind: "$responses" },
      { $match: { "responses.isCorrect": false } },
      {
        $lookup: {
          from: "satassessmentuploads",
    ...
    const feedbacks = await SatFeedback.find({ studentId })
      .sort({ createdAt: -1 })
      .limit(3);
    ...
    for (const subjectKey of Object.keys(subjectMap)) {
      ...
      for (const topicKey of Object.keys(subject.topics)) {
        ...
        try {
          const accuracyData = await SatSubmission.aggregate([
            { $match: { studentId: new mongoose.Types.ObjectId(studentId) } },
            { $unwind: "$responses" },
          ...
          if (accuracyData.length > 0) {
            accuracy = Math.round(
              (accuracyData[0].correct / accuracyData[0].total) * 100
            );
          }
        } catch (err) {
        ...
        const wrongQuestionTexts = topic.questions
          .slice(0, 3)
        ...
        const taskIdBase = `${subject.subject}-${detectedTopic}`
          .replace(/\s+/g, "-")
          .toLowerCase();
        ...
        subjectData.completedMinutes += completedTopicTasks.reduce(
          (sum, t) => sum + t.duration,
          0
        );
        subjectData.totalMinutes += tasks.reduce(
          (sum, t) => sum + t.duration,
          0
        );
      ...
      subjectData.topics.forEach((topic) => {
        ...
        const completion =
          Math.round((completedCount / topic.tasks.length) * 100) || 0;
    ...
    console.error("SAT Study Plan Generation Error:", err);
    res
      .status(500)
      .json({ error: "Failed to generate SAT study plan", details: err.message });
  }
});
...
const getPracticeQuestions = asyncHandler(async (req, res) => {
  try {
    ...
    const questions = await SatAssessmentUpload.aggregate([
      { $unwind: "$questions" },
      {
        $match: {
    ...
    console.error("SAT Practice Questions Error:", err);
    res.status(500).json({ error: "Failed to fetch SAT practice questions" });
  }
});
...
const updateStudyTask = asyncHandler(async (req, res) => {
  try {
    ...
    const task = await SatStudyPlan.findOneAndUpdate(
      { taskId, studentId },
      {
        $set: {
          isCompleted,
    ...
    res.json({
      success: true,
      message: "SAT Task status updated",
      taskId: task.taskId,
      isCompleted: task.isCompleted,
      updatedAt: task.updatedAt,
    });
  } catch (err) {
    console.error("SAT Update Task Error:", err);
    res.status(500).json({ error: "Failed to update SAT task status" });
  }
});
...
const getStudyProgress = asyncHandler(async (req, res) => {
  try {
    ...
    const tasks = await SatStudyPlan.find({ studentId });

    const assessments = await SatSubmission.find({ studentId })
      .sort({ submittedAt: -1 })
      .limit(10);
    ...
    const progressData = {
      ...
      focusAreas: await SatStudyPlan.aggregate([
        { $match: { studentId: new mongoose.Types.ObjectId(studentId) } },
        {
          $group: {
            _id: "$topic",
    ...
    console.error("SAT Study Progress Error:", err);
    res.status(500).json({ error: "Failed to get SAT study progress" });
  }
});
...
const validateSummary = asyncHandler(async (req, res) => {
  try {
    ...
    console.error("SAT Summary validation failed:", err);
    res.status(500).json({
      error: "SAT Summary validation failed",
      isValid: false,
